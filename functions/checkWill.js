  ...
  try {
    ...
    if(c.toLowerCase().includes('--wickets')) {
      ...
      if (!wickets || isNaN(wickets)) {
        wickets = 1;
        await msg.reply('Invalid Value for Flag Wickets and it is set to 1 as default');
      } else if (wickets > 5) {
        wickets = 5;
        await msg.reply('Limited wickets for a duoMatch is 1-5, it is now set to 5');
      }
    }
    if(c.toLowerCase().includes('--overs')) {
      ...
      if (!overs || isNaN(overs)) {
        overs = 5;
        await msg.reply('Invalid Value for Flag Overs, it is set to 5 as default.');
      } else if (overs > 5) {
        overs = 5;
        await msg.reply('Limited overs for a duoMatch is 1-5, it is now set to 5');
      }
    }
